...
<a href="index.php?view=viewgrado&id=<?php echo $_GET["id_grado"]; ?>" class="btn pull-right btn-sm btn-default"><i class='fa fa-arrow-left'></i> Guardar</a>
<?php
//print "<script>window.location='index.php?view=viewgrado&id_grado=$_POS[id]';</script>";
//print "<script>window.location='index.php?view=viewgrado';</script>";
//echo "<script>window.location='index.php?view=viewgrado&id_grado=$_POS[id_grado]';</script>";
?>
<script>
	$("#loadlist").submit(function(e){
		e.preventDefault();
		var d = $("#loadlist").serialize();
		$.get("./?action=loadasistencia",d,function(data){
			console.log(data);
			$("#data").html(data);
